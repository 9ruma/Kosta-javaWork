...
public class BankService {
	...
	final int USER_SIZE = 5; /**고객 최대 개수 */
	static int ACCOUNT_CURRENT_SIZE = 0; /** 현재 계좌의 개수 */
	static int USER_CURRENT_SIZE = 0; /**현재 고객의 개수 */
	...
	public AccountDto[] getAccountList(int userSeq) {
		int searchAcountCount = 0;
		
		for(int i = 0; i < ACCOUNT_CURRENT_SIZE; i++) {
			if(userSeq == accountList[i].getUserSeq()) {
				searchAcountCount++;
		...
		if(searchAcountCount == 0) return null;
		
		AccountDto[] searchAccountDtoList = new AccountDto[searchAcountCount];
		int index = 0;
		
		for(int i = 0; i < ACCOUNT_CURRENT_SIZE; i++) {
			if(userSeq == accountList[i].getUserSeq()) {
				searchAccountDtoList[index++] = accountList[i];
			}
		}
	...
	public UserDto getUserDetail(int userSeq) {
		for(int i = 0; i < USER_CURRENT_SIZE; i++) {
			if(userSeq == userList[i].getUserSeq()) {
